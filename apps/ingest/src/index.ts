...
app.get('/ready', (c) => {
  ...
  if (!ready) {
    return c.json({ status: 'not_ready', reason: 'Missing ClickHouse configuration' }, 503);
  }
...
api.post('/v1/events', zValidator('json', BatchSchema), async (c) => {
  ...
  try {
    ...
    const enrichedEvents = batch.events.map(event => enrichEvent(event, projectId));
    ...
    if (c.env.CLICKHOUSE_URL) {
      ...
      console.log(`[DEV] Received ${enrichedEvents.length} events for project ${projectId}`);
    }
...
async function writeToClickHouse(
  ...
  const rows = events.map(event => ({
    ...
    tool_status: event.type === 'tool_result' 
  ...
  const response = await fetch(`${url}/?query=INSERT INTO events FORMAT JSONEachRow`, {
    method: 'POST',
    headers: {
